...
export default function AdminUsersPage() {
  ...
  const updateMutation = useMutation({ mutationFn: ({ id, req }: { id: number; req: { firstName: string; lastName: string; active: boolean } }) => updateUser(id, req), onSuccess: () => { toast('success','User updated'); invalidate(); setEditItem(null); }, onError: (e) => toast('error', errMsg(e)) });
    ...
    { id: 'createdAt', label: 'Created', render: (_: unknown, row: UserResponse) => row.createdAt ? formatDate(row.createdAt) : '—' },
    {
      ...
      render: (_: unknown, row: UserResponse) => (
        <Stack direction="row" spacing={1}>
          <Button size="small" onClick={() => { setEditItem(row); setForm({ ...form, firstName: row.firstName, lastName: row.lastName }); }}>Edit</Button>
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Button variant="contained" onClick={() => { setForm({ firstName:'',lastName:'',email:'',password:'' }); setCreateOpen(true); }}>Create User</Button>
      </Stack>
      ...
      <FormModal open={!!editItem} title="Edit User" onClose={() => setEditItem(null)} loading={updateMutation.isPending}>
        ...
        <Stack direction="row" justifyContent="flex-end" spacing={1}>
          ...
          <Button variant="contained" onClick={() => editItem && updateMutation.mutate({ id: editItem.id, req: { firstName: form.firstName, lastName: form.lastName, active: editItem.active } })}>Save</Button>
        </Stack>
      </FormModal>
